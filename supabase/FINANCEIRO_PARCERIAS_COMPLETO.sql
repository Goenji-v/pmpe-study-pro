...
begin
  ...
  set valor_aluno_centavos = p_valor_aluno_centavos, atualizado_em = now()
...
begin
  ...
  return jsonb_build_object(
    ...
    'alunos_ativos', coalesce(v_alunos, 0),
    'valor_unitario_centavos', coalesce(v_valor_unitario, 0),
    'estimativa_atual_centavos', coalesce(v_alunos, 0) * coalesce(v_valor_unitario, 0)
  );
end;
$$;
...
begin
  ...
  return coalesce((
    select jsonb_agg(jsonb_build_object(
      ...
      'valor_base_centavos', f.valor_total_centavos,
    ...
    join public.parceiros p on p.id = f.parceiro_id
    where extract(year from f.competencia)::integer = v_ano
  ), '[]'::jsonb);
end;
$$;
