...
begin
  ...
  select valor_aluno_centavos into v_valor_unitario
  from public.parceiros where id = p_parceiro_id;
  if not found then raise exception 'Parceria não encontrada.'; end if;

  select count(*)::integer into v_alunos
  ...
  where parceiro_id = p_parceiro_id and competencia = v_competencia;
  ...
  insert into public.faturamento_parceiros (
    parceiro_id, competencia, alunos_ativos, valor_unitario_centavos,
    fechado_em, fechado_por, status, ajuste_centavos, vencimento_em,
    pago_em, observacao, atualizado_em, atualizado_por
  ) values (
    p_parceiro_id, v_competencia, v_alunos, v_valor_unitario,
    now(), auth.uid(), 'pendente', coalesce(p_ajuste_centavos, 0), p_vencimento_em,
    null, nullif(trim(coalesce(p_observacao, '')), ''), now(), auth.uid()
  )
  ...
  return jsonb_build_object(
    'sucesso', true, 'id', v_id, 'competencia', v_competencia,
    'alunos_ativos', v_alunos, 'valor_unitario_centavos', v_valor_unitario,
...
begin
  ...
  if v_reg.id is null then raise exception 'Fechamento financeiro não encontrado.'; end if;
  return jsonb_build_object(
    'sucesso', true, 'id', v_reg.id, 'status', v_reg.status,
    'valor_devido_centavos', greatest(0, v_reg.valor_total_centavos + v_reg.ajuste_centavos),
...
begin
  ...
  return coalesce((
    select jsonb_agg(jsonb_build_object(
      'id', f.id, 'parceiro_id', f.parceiro_id, 'competencia', f.competencia,
      ...
      'valor_devido_centavos', greatest(0, f.valor_total_centavos + f.ajuste_centavos),
      'status', f.status, 'vencimento_em', f.vencimento_em, 'pago_em', f.pago_em,
      'observacao', f.observacao, 'fechado_em', f.fechado_em, 'atualizado_em', f.atualizado_em
    ) order by f.competencia desc)
    from public.faturamento_parceiros f where f.parceiro_id = p_parceiro_id
  ), '[]'::jsonb);
end;
$$;
...
begin
  ...
  return coalesce((
    select jsonb_agg(jsonb_build_object(
      'id', f.id, 'competencia', f.competencia, 'alunos_ativos', f.alunos_ativos,
    ...
    from public.faturamento_parceiros f where f.parceiro_id = v_parceiro_id
  ), '[]'::jsonb);
end;
$$;
