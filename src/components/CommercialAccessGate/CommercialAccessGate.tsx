...
export default function CommercialAccessGate({ children }: { children: ReactNode }) {
  ...
  if (contexto && !contexto.acessoPermitido) {
    if (PARCERIAS_VISIVEIS && location.pathname === "/meu-acesso") return children;
    return (
      <main className="licenca-bloqueada">
        ...
        {PARCERIAS_VISIVEIS && contexto.parceiroNome && <small>Curso responsável: {contexto.parceiroNome}</small>}
        {contexto.expiraEm && <small>Data de expiração: {new Date(contexto.expiraEm).toLocaleDateString("pt-BR")}</small>}
        {PARCERIAS_VISIVEIS && <Link to="/meu-acesso">Ver detalhes do meu acesso</Link>}
      </main>
    );
  }
  return children;
