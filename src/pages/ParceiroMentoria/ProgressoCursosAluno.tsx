  ...
  const carregar = useCallback(async () => {
    ...
    try {
      ...
      const cursosDoAluno = painel.cursos.filter((curso) => {
        if (!curso.ativo || curso.status !== "publicado") return false;
    ...
    <section className="mentoria-aluno-card curso-aluno-bloco">
      <div className="mentoria-aluno-card-topo compacto">
        <div>
          ...
          <p>Mostra o avanço real nas aulas publicadas e liberadas para a turma deste aluno.</p>
        </div>
      ...
      {carregando ? (
        ...
        <p className="mentoria-aluno-vazio">Nenhum curso publicado está liberado para este aluno.</p>
      ) : (
                  ...
                  {curso.concluidasRecentes.map((aula) => (
                    <div key={aula.id}>
                      ...
                      <div>
                        ...
                        <small>
                          {aula.disciplina} · {aula.modulo}
                          {aula.concluidaEm ? ` · ${formatarData(aula.concluidaEm)}` : ""}
                        </small>
                      </div>
                    </div>
                  ))}
