...
function urlApi(caminho: string) {
...
export async function analisarProvaOficial(args: {
  ...
  const [provaBase64, gabaritoBase64] = await Promise.all([
    arquivoParaBase64(args.prova),
    arquivoParaBase64(args.gabarito),
  ]);
  ...
  const resposta = await fetchApiAutenticada(urlApi("/api/analisar-prova"), {
    ...
    body: JSON.stringify({
      ...
      metadados: {
        concursoAlvo: args.concursoAlvo,
        editalAlvo: args.editalAlvo,
        concursoOrigem: args.concursoOrigem,
        cargoOrigem: args.cargoOrigem,
        anoOrigem: args.anoOrigem,
        banca: args.banca,
      },
  ...
  if (!resposta.ok || !dados.sucesso || !dados.analise) {
    throw new Error(dados.erro || "Não foi possível analisar a prova.");
  }
...
export async function criarSimuladoOficial(args: {
  ...
  fonteProvaNome: string;
  fonteGabaritoNome: string;
  ...
  const { data: simulado, error } = await supabase
    ...
    .insert({
      nome: args.nome,
      concurso_alvo: args.concursoAlvo,
      edital_alvo: args.editalAlvo,
      banca: args.banca,
      data_prova: args.dataProva || null,
      duracao_minutos: args.duracaoMinutos,
      total_questoes: args.questoes.length,
      status: "rascunho",
      fonte_prova_nome: args.fonteProvaNome,
      fonte_gabarito_nome: args.fonteGabaritoNome,
    })
    .select("*")
    .single();
  ...
  const questoes = args.questoes.map((questao) => ({
    id: questao.id,
    ...
    modulo_id: questao.modulo_id || null,
    ...
    ordem: questao.ordem || questao.numero,
  }));
  ...
  const gabaritos = args.questoes
    .filter((questao) => ["A", "B", "C", "D", "E"].includes(questao.respostaCorretaId || ""))
    .map((questao) => ({
      simulado_id: simulado.id,
      numero: questao.numero,
      resposta: questao.respostaCorretaId,
      anulada: false,
    }));

  const anuladas = args.questoes
    .filter((questao) => !questao.respostaCorretaId)
    .map((questao) => ({
      simulado_id: simulado.id,
      numero: questao.numero,
      resposta: "A",
  ...
  const { error: erroGabarito } = await supabase
    ...
    .insert([...gabaritos, ...anuladas]);

  if (erroGabarito) {
    await supabase.from("simulados_oficiais").delete().eq("id", simulado.id);
    throw new Error(`Erro ao salvar gabarito: ${erroGabarito.message}`);
  }
...
export async function finalizarTentativaOficial(tentativaId: string, respostas: Record<string, string>, minutosGastos: number) {
  const { data, error } = await supabase.rpc("finalizar_simulado_oficial", {
    ...
    p_minutos_gastos: Math.max(0, Math.round(minutosGastos)),
  });
  if (error) throw new Error(error.message);
